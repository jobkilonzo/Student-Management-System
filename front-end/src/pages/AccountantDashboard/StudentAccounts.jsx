      ...
      <div className="space-y-6">
        <section className="bg-white rounded-2xl shadow-sm p-6">
          ...
          <p className="text-slate-500 text-sm mt-1">
            Select a student, enter the amount paid, and the system will deduct it from the outstanding balance.
          </p>

          <form onSubmit={handleRecordPayment} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4 mt-5">
            <select
          ...
          {selectedAccount && (
            <div className="mt-4 rounded-xl bg-slate-50 p-4 text-sm text-slate-600">
              ...
              <span className="font-semibold text-rose-600">{formatCurrency(selectedAccount.balance)}</span>
            </div>
          )}
        </section>
              ...
              <tbody>
                {filteredAccounts.map((account) => (
                  <tr key={account.id} className="border-b last:border-b-0 hover:bg-slate-50">
                    ...
                    <td className="py-4 font-semibold text-rose-600">{formatCurrency(account.balance)}</td>
